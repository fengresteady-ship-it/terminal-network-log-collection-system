...
本系统是一款面向 Windows的网络日志采集与分析系统，旨在为安全测试、威胁检测和应急响应提供完整的主机行为记录和网络流量取证能力。
...
- **双维度采集** — 同时采集 Sysmon 主机事件日志和网络流量数据包，可以通过时间对齐进行交叉印证
...
**API 远程控制：**
```bash
curl -X POST http://192.168.0.1:9880/api/mission/start \
  -H "Content-Type: application/json" \
  -d '{"iface":"8"}'
# 执行测试...
curl -X POST http://192.168.0.1:9880/api/mission/stop
```
